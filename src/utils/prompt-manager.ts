 ...
 * 更新自定义提示词
 */
export function updateCustomPrompt(id: string, updates: Partial<Pick<CustomPrompt, 'name' | 'prompt'>>): boolean {
  try {
    const customPrompts = loadCustomPrompts();
    ...
    // 检查新名称是否与其他提示词冲突
    if (updates.name && customPrompts.some((p, index) => p.name === updates.name && index !== promptIndex)) {
...
export function findPromptById(id: string): PromptOption | null {
  ...
  return allPrompts.find(p => 
    p.isCustom ? p.id === id : p.key === id
  ) || null;
}
 ...
 * 验证自定义提示词的输入
 */
export function validateCustomPrompt(name: string, prompt: string): { valid: boolean; error?: string } {
  if (!name.trim()) {
